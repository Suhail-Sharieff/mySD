...
public class _08_UnisexBathroom {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FairBathRoom4.main(args);
    }

    // ------------untill one's count doesnt become 0, other shud wait ie unfair to
    // any one ie starvation OCCURS here
    ...
    // ------unfair bathroom, usig ConditionalVariables: learn hwo actions are
    // segreated, initilly i put all logic inside maleEnter and femleEner, it was
    // very complex, so separet into function, each for male and female define 2
    // func acquire and reease
    /*
    ...
    private static class UnFairBathRoom2 {
        ...
        private final Condition canEnter;// MISTAKE: took 2 conditions: maleEntry and femaleEntry: thats wrong, just
                                         // imagine it as layer with 1 door, and u shud decide if u can enter it or not
                                         // so 1 cond variable, 2 cond variable wud mean that there are 2 doors thats
                                         // wrong
        ...
        private void maleAcquire() throws InterruptedException {
            lock.lock();
            try {
                while (currGender == Gender.FEMALE)
                    canEnter.await();// in future u can also limit nPPl by adding nPPl too, for now i hv skipped it
                                     // to keep it simple
                n_PPl_In_Bathroom++;
                currGender = Gender.MALE;
                // NO need of canEntrer.sigalAll() here coz other threads will still be waiting
                // on lock.lock() and woulnt have touched while(...)
            } finally {
                lock.unlock();
            }
        }
    ...
    // -------------------same fair bathroom, but cleaner code version (separated
    // functions), it helps to segrefate reponsitbiities very cleanly, in future
    // supose we want to add anoher gender, then we just need to create private void
    // newGenderAcuire()..release() and then public newGenderENtter()

    /*
     * 
     * here i hv added timings too to visualize, male taks 2s and female taks 3s in
     * bathroom
     * 
    ...
    // ----------------------fair bathroom with limit on how many in bathroom at a
    // time---uses same approach used in ./_05_ReadHeavy.java
    /*
     * 
    ...
    // there is Solution using Lock+Condtional varible too, but its way too
    // complicated coz we need to applyconditionals on mnay things, below is sameple
    // code, but prefer Semaphore based soln as its clean and easy to understadn

    private static class FairBathRoom4 {
        public static void main(String[] args) throws Exception {
            Thread males[] = new Thread[10];
            Thread females[] = new Thread[10];
            FairBathRoom4 obj = new FairBathRoom4();
            ...
            for (int i = 0; i < Math.max(males.length, females.length); i++) {
                if (i < males.length)
                    males[i].start();
                if (i < females.length)
                    females[i].start();
            ...
            for (Thread m : males)
                m.join();
            for (Thread f : females)
                f.join();
        }

        public FairBathRoom4() {
            room = lock.newCondition();
        }
        ...
        // waiting counters, if v dont maintian them and just rely on inside varibale, we will get like MALE->FEMALE->MALE->FEMALE... ie its like 1 person at a time and NOT batch wise
        private int waitingMales = 0;
        private int waitingFemales = 0;

        // You can keep it, but it's no longer critical
        private final Semaphore queue = new Semaphore(1, true);

        void maleAcquire() throws InterruptedException {
            queue.acquire();
            ...
            try {
                ...
                while (currGender == Gender.FEMALE ||
                        (currGender == Gender.MALE && waitingFemales > 0)) {
                    room.await();
                }
        ...
        void femaleAcquire() throws InterruptedException {
            queue.acquire();
            ...
            try {
                ...
                while (currGender == Gender.MALE ||
                        (currGender == Gender.FEMALE && waitingMales > 0)) {
                    room.await();
                }
